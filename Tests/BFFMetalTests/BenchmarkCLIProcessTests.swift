...
final class BenchmarkCLIProcessTests: XCTestCase {
    ...
    /// schemaVersion-4 envelope with an empty `results` array and exits 2. On a Metal
    /// host the same invocation actually runs, so we only require schemaVersion 4 and a
    ...
    func testNoSamplesFlagPropagatesAndSchemaIsStable() throws {
        ...
        let obj = try JSONSerialization.jsonObject(with: Data(r.stdout.utf8)) as? [String: Any]
        XCTAssertEqual(obj?["schemaVersion"] as? Int, 4, "schemaVersion 4 emitted")
