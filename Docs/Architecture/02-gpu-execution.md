...
| **v0.5 (grounding)** | **DONE** ([../CubffGrounding.md](../CubffGrounding.md)): six alignment tags (01 §7.4) confirmed against pinned cubff source; genuine evaluator fixtures generated by executing the unmodified pinned evaluator, enforced in `swift test`. Whole-soup `cubffCompat` replay (01 §7.1) remains optional | Oracle reproduces every cubff evaluator observable **bit-identically** on all fixtures; all six tags recorded as confirmed/corrected in 06 D2 |
