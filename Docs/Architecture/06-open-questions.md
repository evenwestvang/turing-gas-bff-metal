...
| D2 | Exact cubff alignment: the six pinned tags of **01 §7.4** (opcode bytes, init pc/heads, mutate order, step counting, loop re-entry landing, `CheckSelfRep` accounting) | **RESOLVED** (2026-07, [../CubffGrounding.md](../CubffGrounding.md)): all six confirmed from cubff source at commit `f212e84`, enforced by genuine evaluator fixtures (`CubffFixtureTests`). Findings: tags 1/2/4/5 as assumed (tag 4 gained the `commandSteps` refinement — cubff reports ops excluding comment bytes); tag 3 semantics confirmed but cubff's order is pair→mutate→run (composition-equivalent; oracle order retained); tag 6 corrected — offspring half feeds forward, agreement threshold is ≥4-of-13. cubff has **no jump table** — dynamicScan is the cubff-normative mode (feeds D1). Full bit-parity of *our* production RNG trajectories with cubff remains a non-goal. |
